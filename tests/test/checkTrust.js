...
describe("CHECK-TRUST", function () {
  ...
  describe("CheckTrust Test Suite", function () {
    ...
    it("Should not be able to get user when country code is invalid too long", async function () {
      // Given
      const code = "00000";
      const phone = "62614370";
      ...
      expect(response.body.message).to.equal(
        "(603) country_code in path should be at most 4 chars long"
      );
    });

    it("Should not be able to get user when phone number is invalid too long", async function () {
      // Given
      const code = "370";
    ...
    it("Should not be able to get user when phone number is empty", async function () {
      ...
      expect(response.body.code).to.equal("VALIDATION_ERROR");
      expect(response.body.message).to.equal(
        "(404) path /mth/v1/users/phone/370/ was not found"
      );
    });
    ...
    it("Should not be able to get user when country code is empty", async function () {
      ...
      expect(response.body.code).to.equal("VALIDATION_ERROR");
      expect(response.body.message).to.equal(
        "(404) path /mth/v1/users/phone/62614370 was not found"
      );
    });
  });
});
